...
namespace PLAYLIST
{
    public partial class MainWindow : Window
    {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            // Crea una nueva instancia de Song y agrégala a la colección Songs
                            Songs.Add(new Song { SongTitle = title, ArtistName = genre });
                        }
                    }
                }
            }
        ...
        private void Remove_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (selectedSong != null)
            {
                MessageBoxResult result = MessageBox.Show($"¿Estás seguro de que deseas eliminar '{selectedSong.SongTitle}'?", "Confirmar eliminación", MessageBoxButton.YesNo, MessageBoxImage.Question);

                if (result == MessageBoxResult.Yes)
                {
                    // Elimina la canción de la lista Songs
